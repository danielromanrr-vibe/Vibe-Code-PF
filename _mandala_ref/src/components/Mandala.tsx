...
const getRandomColor = () => {
  // Generate vibrant colors using HSL logic but returning RGB for easier interpolation
  const h = Math.random();
...
export default function Mandala() {
  ...
  useEffect(() => {
    ...
    const handleMouseDown = () => {
      ...
      // Randomize palette on every press
    ...
    const render = () => {
      ...
      // Use the ref-based palette that updates on press
      ...
      if (!state.isGrabbed) {
        if (state.placedPos) {
          ...
          const margin = 200;
          if (targetY < -margin || targetY > window.innerHeight + margin ||
      ...
      // Technical Grid Overlay (Blueprint feel)
      if (hf > 0.1 || pf > 0.1) {
        ctx.save();
      ...
      const numLayers = 20; // Increased layers for more density
      for (let i = 0; i < numLayers; i++) {
        ...
        const accentRGB = activePalette[i % activePalette.length].match(/\d+/g)!.map(Number);
        const targetColor = { r: accentRGB[0], g: accentRGB[1], b: accentRGB[2] };
        
        // Progressive color shift based on both hover and press
        ...
        // Technical Rails & Atmospheric Traces
        if ((pf > 0.2 || hf > 0.5) && i % 5 === 0) {
          ...
          ctx.beginPath();
          ctx.moveTo(0, lcy); ctx.lineTo(canvas.width, lcy);
          ctx.moveTo(lcx, 0); ctx.lineTo(lcx, canvas.height);
          ctx.stroke();
