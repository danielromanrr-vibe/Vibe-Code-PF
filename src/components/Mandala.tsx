...
export default function Mandala() {
    ...
    const dropMandala = () => {
      ...
      if (home) {
        ...
        const distToHome = Math.sqrt(
          Math.pow(mouseRef.current.x - homeX, 2) + Math.pow(mouseRef.current.y - homeY, 2)
        );
  ...
  return (
    <canvas
      ...
      className={`fixed inset-0 w-full h-full block ${isHovered || isGrabbedState ? 'z-[100] pointer-events-auto' : 'z-[10] pointer-events-none'}`}
